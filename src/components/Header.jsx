...
const navItemsInfo=[
  {name:"الرئيسية",type:"link",href:'/'},
  { name: "الاخبار", type: "link", href: "/blog" },
  {name:"الصفحات",type:"dropdown",items:[{title:"الطقس",href:"/weather"},{title:"مواقيت الصلاة",href:"/prayer"}]},
  {name:"الاسئلة الشائعة",type:"link",href:'/feq'},
  ...
  <section className="sticky top-0 left-0 right-0 z-50 bg-white ">
    <header className='container mx-auto px-5 flex justify-between py-4 items-center'>
      ...
      <div
        ...
        >
        <ul className='text-white items-center gap-y-5 lg:text-dark-soft flex flex-col lg:flex-row gap-x-2 font-semibold font-sans'>
        ...
        <div className='text-white items-center gap-y-5 lg:text-dark-soft flex flex-col lg:flex-row gap-x-2 font-semibold'>
            ...
            <button  className='flex gap-x-1 items-center mt-5 lg:mt-0 border-2 border-blue-500 px-6 py-2 rounded-full text-blue-500 font-semibold hover:bg-blue-500 hover:text-white transition-all duration-300'
             onClick={()=>setProfileDrowpdown(!profileDrowpdown)}>
              <span className="font-sans">الحساب</span>
              <MdKeyboardArrowDown/>
              </button>
              ...
              <ul className="bg-dark-soft lg:bg-transparent text-center flex flex-col shadow-lg overflow-hidden">
                    {userState?.userInfo?.admin && (
                        <button
                          ...
                          type="button"
                          className="font-sans hover:bg-dark-hard hover:text-white px-4 py-2 text-white lg:text-dark-soft"
                        >
                         لوحة التحكم 
                        </button>
                      )}
                    <button onClick={()=>navigate('/profile')} type="button"  className="font-sans hover:bg-dark-hard hover:text-white px-4 py-2 text-white lg:text-dark-soft">
                      الصفحة الشخصية 
                    </button>
                    <button onClick={logoutHandler} type="button"  className="font-sans hover:bg-dark-hard hover:text-white px-4 py-2 text-white lg:text-dark-soft">
                      تسجيل الخروج
                    </button>
          ...
          <button onClick={()=>navigate('/login')} className='mt-5 border-2 border-blue-500 px-6 py-2 rounded-full text-blue-500 font-semibold hover:bg-blue-500 hover:text-white transition-all duration-300 font-sans'>تسجيل الدخول </button>
            )}
      </div>
    </header>
  </section>
